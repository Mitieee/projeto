        ...
        <div class="emergencias-lista" id="emergencias-lista">
            <?php foreach ($emergencias as $emergencia): ?>              
                <div class="emergencia-card <?= $emergencia['id'] == $id_emergencia ? 'selecionada' : '' ?>" 
                     onclick="window.location.href='usuario2teste.php?id_emergencia=<?= $emergencia['id'] ?>'">
...
<script async defer src="https://maps.googleapis.com/maps/api/js?key=<?php echo $apiKey; ?>&callback=initMap"></script>
    <script>     
        ...
        function atualizarLocalizacaoUsuario1() {
            fetch(`usuario2teste.php?obter_localizacao_usuario1=1&id_emergencia=<?= $id_emergencia ?>`)
        ...
        function atualizarChat() {
            fetch(`usuario2teste.php?id_emergencia=<?= $id_emergencia ?>&obter_mensagens=1`)
                .then(response => response.json())
                .then(mensagens => {
        ...
        function enviarStatus(status) {
            fetch('usuario2teste.php?status_acao=1', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        ...
        function verificarStatus() {
            fetch(`usuario2teste.php?status_acao=1&id_emergencia=<?= $id_emergencia ?>`)
                .then(response => response.json())
                .then(status => {
